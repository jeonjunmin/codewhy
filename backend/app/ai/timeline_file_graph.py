...
- 각 커밋 줄 끝의 ⟪실제변경 …⟫ 은 git diff 에서 뽑은 '실제 코드 변경'(사실)입니다 — 바뀐 줄 수와 함수/섹션명.
  · 커밋 메시지는 '의도'를, ⟪실제변경⟫ 은 '실제로 바뀐 것'을 알려줍니다. 둘이 어긋나면 무엇이 바뀌었는지는 ⟪실제변경⟫ 을 우선해 판단하세요.
  · 개발자가 메시지를 잘못/대충 적었어도(예: "오타 수정"인데 함수 대거 추가) ⟪실제변경⟫ 으로 교정해 요약하세요. '왜(의도)'만 메시지에서 취합니다.
  · ⟪실제변경⟫ 이 없는 커밋은 메시지만으로 판단하세요. ⟪실제변경⟫ 의 함수명을 요약에 그대로 나열하지 말고, 무슨 변화였는지로 풀어 쓰세요.

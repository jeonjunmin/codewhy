...
<body>${body}</body>
    ...
    private _body(): string {
        ...
        if (s.kind === 'streaming') {
            ...
            // 스트리밍 중에는 미완성 토큰에 renderBold(마크다운 처리)를 적용하지 않는다.
            // esc() 로만 HTML 특수문자를 이스케이프해 검은 배경 등 스타일 오작동을 방지.
...
<div class="ai-card">
  ...
  <div class="ai-text streaming-text">${esc(s.text)}<span class="caret"></span></div>
</div>
</div>`;
        }
...
${milestoneHtml}
    ...
    private _css(): string {
        return `
/* ── VSCode 웹뷰 주입 CSS 무력화 ─────────────────────────────────────────────
   VSCode는 웹뷰에 기본 테마 변수(--vscode-textPreformat-background 등)를 심고
   code/pre/kbd/samp 등에 어두운 배경을 적용한다. CSS 변수 자체를 :root와
   전체 선택자에서 transparent 로 재선언해 어떤 선택자 우선순위로도 뚫리지 않도록 한다. */
:root {
  --vscode-textPreformat-background: transparent !important;
  --vscode-textPreformat-foreground: inherit !important;
  --vscode-textBlockQuote-background: transparent !important;
  --vscode-textBlockQuote-border: transparent !important;
  --vscode-textCodeBlock-background: transparent !important;
}
html, body {
  --vscode-textPreformat-background: transparent !important;
  --vscode-textBlockQuote-background: transparent !important;
...
/* ── 어두운 배경 완전 차단 ───────────────────────────────────────────────────
   .ai-text 하위의 모든 요소, 그리고 스트리밍 컨테이너 하위의 모든 요소에 대해
   background 를 transparent 로 강제한다. !important 체인을 끊기 위해
   개별 태그 선택자도 병기해 명시도(specificity)를 높인다. */
.ai-text *,
.streaming-text *,
.ai-text code, .ai-text pre, .ai-text kbd, .ai-text samp, .ai-text blockquote,
.streaming-text code, .streaming-text pre, .streaming-text kbd, .streaming-text samp {
  background: transparent !important;
  background-color: transparent !important;
  font-family: inherit !important;
  color: inherit !important;
...
.ai-text strong, .ai-text b { color: #fff; font-weight: 700; background: transparent !important; }
/* 스트리밍 중: 줄바꿈 보존 + 배경 이중 차단 */
.streaming-text {
...
.caret {
  ...
  background: var(--vscode-editor-foreground) !important;
